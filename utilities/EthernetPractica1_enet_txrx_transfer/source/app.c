...
int main(void)
{
    uint8_t *message = (uint8_t*)malloc(MAX_MSG);
	status_t status_phy = security_ENET_INIT();
	status_t status;
    ...
    do
    {
        ...
        {
        	PRINTF("\e[1;1H\e[2J");
        	security_send(message, strlen(message));
        	do{
        		status = security_receive();
        	}while(status);

        }


        PRINTF("\r\nDeseas mandar otro mensaje 1)si 0)no \r\n ");
        repetir = GETCHAR();
        	if(repetir == '0')
        		repetir = 0;
    }while(repetir);
